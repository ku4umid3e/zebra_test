...
|-------------|---------|-------|-------|-------|----------|-------------|----------|-------------|------|
| 1 (Базовая) | 45 изображений | 200 | 8 | ~47ч | 0.9519 | 0.8106 | 0.9250 | 0.7840 | Автоаннотация |
...
Сохранены следующие графики в `runs/segment/train/`:
- `results.png` - График метрик обучения
- `confusion_matrix.png` - Матрица ошибок
- `MaskP_curve.png` - Кривая точности для масок
- `MaskR_curve.png` - Кривая полноты для масок
- `MaskF1_curve.png` - F1-кривая для масок

## 6. Сложности и их решения

### 6.1 Технические сложности
- **Ограниченные вычислительные ресурсы:** Обучение проводилось на CPU
...
3. **Автоматизация развертывания** CVAT через Docker Compose
4. **CI/CD pipeline** для автоматического обучения при добавлении данных
5. **Создание тестового набора** для финальной оценки модели
6. **Экспериментирование** с YOLOv11m/l/x для повышения точности

### 7.3 Практическое применение
Модель готова для использования в:
- Системах автоматического распознавания продуктов питания
- Мониторинге качества пищи в ресторанах
- Инвентаризации продуктов на складах
...
- **Ручная разметка 45 изображений:** ~8 часов  
- **Обучение базовой модели (200 эпох):** ~47 часов
- **Настройка AutoCVAT и автоаннотация:** ~6 часов
- **Конвертация и подготовка финального датасета:** ~2 часа
- **Обучение финальной модели (20 эпох):** ~4.6 часа
- **Анализ результатов и написание отчета:** ~8 часов

**Активное время разработки:** ~20 часов  
**Время ожидания обучения:** ~40 часов

## 9. Опыт работы с YOLO

**Предыдущий опыт:** Минимальный опыт
